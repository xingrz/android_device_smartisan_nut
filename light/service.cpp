...
#define LOG_TAG "android.hardware.light@2.0-service.nut"
...
const static std::string kRedOnTimePath = "/sys/class/leds/red/ontime";
const static std::string kRedPeriodPath = "/sys/class/leds/red/period";
const static std::string kRedRampTimePath = "/sys/class/leds/red/ramp_time";
...
int main() {
    ...
    std::ofstream redOnTime(kRedOnTimePath);
    if (!redOnTime) {
        LOG(ERROR) << "Failed to open " << kRedOnTimePath << ", error=" << errno
                   << " (" << strerror(errno) << ")";
        return -errno;
    }

    std::ofstream redPeriod(kRedPeriodPath);
    if (!redPeriod) {
        LOG(ERROR) << "Failed to open " << kRedPeriodPath << ", error=" << errno
                   << " (" << strerror(errno) << ")";
        return -errno;
    }

    std::ofstream redRampTime(kRedRampTimePath);
    if (!redRampTime) {
        LOG(ERROR) << "Failed to open " << kRedRampTimePath << ", error=" << errno
                   << " (" << strerror(errno) << ")";
        return -errno;
    }
    ...
    android::sp<ILight> service = new Light(
            ...
            std::move(redOnTime), std::move(redPeriod), std::move(redRampTime),
            std::move(redBlink));
